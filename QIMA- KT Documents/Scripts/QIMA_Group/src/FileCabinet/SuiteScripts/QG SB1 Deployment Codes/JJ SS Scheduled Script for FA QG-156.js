 ...
 */
define(['N/email', 'N/error', 'N/record', 'N/search','N/runtime','N/task','N/currentRecord','N/format'],
    /**
     * @param{email} email
     * @param{error} error
     * @param{record} record
     ...
     * @param{format} format
     */
    (email, error, record, search,runtime,task,currentRecord, format) => {
        ...
        function journalFACreation(paramContext) {
            try{
                ...
                if(jRes.length>0) {
                    ...
                    log.debug("JOURNAL RES: ",journalRes)
                    log.debug("JOURNAL LENGTH: ",journalRes.length)

                    var scriptObj = runtime.getCurrentScript();
                    var start = scriptObj.getParameter({name: 'custscript_start_range'})
                    var end = scriptObj.getParameter({name: 'custscript_end_range'})
                    var tot = scriptObj.getParameter({name: 'custscript_tot_count'})

                    if(checkForParameter(start)==false){
                        start = 0
                    }
                    if(checkForParameter(tot)==false){
                        tot = Number(journalRes.length)
                    }
                    if(checkForParameter(end)==false){
                        if(tot>100){
                            end = 100
                        }
                        else{
                    ...
                    if (tot > 0) {
                        for (var j = start; j < end; j++) {
                            ...
                            log.debug("REMAINING USAGE: ", remaingUsage)
                            if (remaingUsage < 500) {
                                rescheduleScriptandReturn(j,100,tot)
                            }
                            else {
        ...
        function vendorFACreation(paramContext){
            try{
                                        ...
                                        for (var i = 0; i < ex; i++) {
                                            ...
                                            if (accType == 'FixedAsset') { // if the account type is Fixed Asset
                                                ...
                                                if ((checkForParameter(exAsset) == true) && (checkForParameter(relRec) == false)) {
                                                    ...
                                                    var assetAccount = assettypeRec.getValue({
                                                    ...
                                                    if((checkForParameter(assetAccount)==true) && (assetAccount == exAcc)) {
                ...
                log.error({
                    title: "Error @ vendorFACreation: ",
                    details: e.name+' : '+e.message
                })
            }
        }

        function rescheduleScriptandReturn(startRange,limit,total) {
            try{
                log.debug("RESCHEDULE START RANGE: ",startRange)
                log.debug("RESCHEDULE LIMIT: ",limit)
                log.debug("RESCHEDULE TOTAL: ",total)
                var endRange = Number(startRange)+Number(limit)
                if(endRange>total){
                    endRange = Number(total)
                }
                log.debug("RESCHEDULE END RANGE: ",endRange)
                var ssTask = task.create({
                    ...
                    params: {
                        ...
                        custscript_end_range: endRange,
                        custscript_tot_count: total
                    }
                });
