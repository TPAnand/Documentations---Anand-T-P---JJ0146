 ...
 */
    (record, runtime, search, task, serverWidget,file) => {
        ...
        var CLIENT_SCRIPT_ID = 68991; // Sandbox file ID
        ...
        const setSublistValuesFromCustomerDetails = (CustomerDetails,sublistForCustomers) => {
            try{
                ...
                for(var i=0;i<CustomerDetails.length; i++){
                    ...
                    //     line:i,
                    //     value:CustomerDetails[i]['customerName']
                    // })
                    sublistForCustomers.setSublistValue({
                        id:'custpage_customer_name',
                        line:i,
        ...
        const getCustomerData = (parameters) => {
            try{
                ...
                if(customerGrade && customerGrade!=0){
                    ...
                    filterArray.push(["custrecord155.custrecord_jj_customer_grades_planogram", "anyof",customerGrade ]);
                }
                if(trainingGrade && trainingGrade!=0){
                    if(filterArray.length>0){
                        filterArray.push("AND");
                    }
                    filterArray.push(["custrecord155.custrecord_jj_customer_grades","anyof",trainingGrade]);
                }
        ...
        function taskTypeList(){
            try{
                var customlist_jj_task_type_listSearchObj = search.create({
                    type: "customlist_jj_task_type_list",
                    filters:
                        [
                        ],
                    columns:
            ...
            try{
                ...
                if(scriptContext.request.method=='GET' && mode!='page4'){
                   ...
                   var prioritySublist= sublistForCustomers.addField({
                    ...
                    // starttimePickerSublist.updateLayoutType({layoutType: serverWidget.FieldLayoutType.MIDROW})
                    // starttimePickerSublist.updateDisplayType({displayType:serverWidget.FieldDisplayType.DISABLED})
                    ...
                    log.debug("scriptContext.request.method: ",scriptContext.request.method)
